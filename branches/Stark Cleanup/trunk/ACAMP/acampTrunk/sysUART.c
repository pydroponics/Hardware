...
Description:
	...
	on receipt of a byte, and reads the receive buffer (UDR0).
...
ISR (USART0_RX_vect) {
	...
	//UDR0 = rxDataBuffer0[piDataIndex];
...
Description:
    ...
    receive array. Then it checks for end of line '\r' and sets the ready
    flag accordingly.
...
ISR (USART1_RX_vect) {
	...
	if (rxDataBuffer1[tdsDataIndex] == '\r') {
		tdsDataReady = TRUE;
		tdsDataIndex = 0;
	} else {
		tdsDataIndex++;
