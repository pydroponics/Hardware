...
void sysTransmit(char txData[], unsigned int txLength, unsigned int usartIndex, unsigned int ackReq) {
	switch (usartIndex) {
		case 0:
			sysCopyArray(txData, txLength, txDataBuffer0, txLength);
			UCSR0B |= (1<<UDRIE0);
			if(ackReq == ON) {
				UCSR0B |= (1<<RXCIE0);
				ackReqFlag0 = ON;
...
ISR (USART0_RX_vect) {
	...
	//test of raspi
	if(rxDataBuffer0[0] == 'p' || rxDataBuffer0[0] == 'P'){
		sysTransmit("pH: \n", 5, 0, OFF);
	}
	if(rxDataBuffer0[0] == 't' || rxDataBuffer0[0] == 'T'){
		sysTransmit("tds: \n", 6, 0, OFF);
	}
	if(rxDataBuffer0[0] == 'i' || rxDataBuffer0[0] == 'I'){
		sysTransmit("fan increase\n", 13, 0, OFF);
	}
	...
	if (rxData == 0x0D) {
		...
		sysTransmit(rxDataBuffer0, MAX_TRANSMIT_LENGTH, 0, OFF);
		sysTransmitChar('\n',0,OFF);
		sysZeroArray(rxDataBuffer0, MAX_TRANSMIT_LENGTH);
